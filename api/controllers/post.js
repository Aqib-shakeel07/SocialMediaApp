...
db.query(deleteLikesQuery, [req.params.id], (commentsErr, commentsData) => {
  if (commentsErr) {
...
db.query(deleteCommentsQuery, [req.params.id], (commentsErr, commentsData) => {
  if (commentsErr) {
    return res.status(500).json(commentsErr);
  }
  ...
  // Delete the post regardless of whether comments were found or not
  db.query(deletePostQuery, [req.params.id, userInfo.id], (postErr, postData) => {
    if (postErr) {
